# Re-review tracker

Procedure, standard, and the prompt to paste: [README.md](README.md). One row
per request; tick it only when every step of the README's loop is done and
committed. Fill `Fixed` with the defect count (`—` for a row closed by the
September 22 full-scope sweep, whose counts are in `docs/history/`), `Errata` with the numbers
logged (or `—`), and `Commit` with the short hash.

Status: `[ ]` pending · `[~]` in progress (note the section reached) · `[x]` done

## Reference: Anatomy and Physiology 2e

Chapters 1–2 are the standard (authored with the full kit and re-reviewed by
Opus on September 22, 2026). New A&P chapters meet it by being authored with
`docs/briefs/anatomy-physiology/run.md`, so they are not tracked here.

## Prealgebra 2e

`content/math/prealgebra` · 11 chapters, 60 section pages, 2 knowledge checks

Never read against the September 22 bar: needs the full checker read AND
the image-first figure pass. The first math request is the pilot (README
step 0).

| | Chapter | Sections | Fixed | Errata | Commit | Notes |
|---|---|---|---|---|---|---|
| [ ] | 1. Whole Numbers | 5 | | | | |
| [ ] | 2. The Language of Algebra | 5 | | | | |
| [ ] | 3. Integers | 5 | | | | |
| [ ] | 4. Fractions | 7 | | | | |
| [ ] | 5. Decimals | 7 | | | | |
| [ ] | 6. Percents | 5 | | | | |
| [ ] | 7. The Properties of Real Numbers | 5 | | | | |
| [ ] | 8. Solving Linear Equations | 4 | | | | |
| [ ] | 9. Math Models and Geometry | 7 | | | | |
| [ ] | 10. Polynomials | 6 | | | | |
| [ ] | 11. Graphs | 4 | | | | |
| [ ] | KC `knowledge-check-01-06` | — | | | | |
| [ ] | KC `knowledge-check-07-11` | — | | | | |

## Elementary Algebra 2e

`content/math/elementary-algebra` · 10 chapters, 71 section pages, 2 knowledge checks

Never read against the September 22 bar: needs the full checker read AND
the image-first figure pass. The first math request is the pilot (README
step 0).

| | Chapter | Sections | Fixed | Errata | Commit | Notes |
|---|---|---|---|---|---|---|
| [ ] | 1. Foundations | 10 | | | | |
| [ ] | 2. Solving Linear Equations and Inequalities | 7 | | | | |
| [ ] | 3. Math Models | 6 | | | | |
| [ ] | 4. Graphs | 7 | | | | |
| [ ] | 5. Systems of Linear Equations | 6 | | | | |
| [ ] | 6. Polynomials | 7 | | | | |
| [ ] | 7. Factoring | 6 | | | | |
| [ ] | 8. Rational Expressions and Equations | 9 | | | | |
| [ ] | 9. Roots and Radicals | 8 | | | | |
| [ ] | 10. Quadratic Equations | 5 | | | | |
| [ ] | KC `knowledge-check-01-05` | — | | | | |
| [ ] | KC `knowledge-check-06-10` | — | | | | |

## Intermediate Algebra 2e

`content/math/intermediate-algebra` · 12 chapters, 70 section pages, 2 knowledge checks

Never read against the September 22 bar: needs the full checker read AND
the image-first figure pass. The first math request is the pilot (README
step 0).

| | Chapter | Sections | Fixed | Errata | Commit | Notes |
|---|---|---|---|---|---|---|
| [ ] | 1. Foundations | 5 | | | | |
| [ ] | 2. Solving Linear Equations | 7 | | | | |
| [ ] | 3. Graphs and Functions | 6 | | | | |
| [ ] | 4. Systems of Linear Equations | 7 | | | | |
| [ ] | 5. Polynomials and Polynomial Functions | 4 | | | | |
| [ ] | 6. Factoring | 5 | | | | |
| [ ] | 7. Rational Expressions and Functions | 6 | | | | |
| [ ] | 8. Roots and Radicals | 8 | | | | |
| [ ] | 9. Quadratic Equations and Functions | 8 | | | | |
| [ ] | 10. Exponential and Logarithmic Functions | 5 | | | | |
| [ ] | 11. Conics | 5 | | | | |
| [ ] | 12. Sequences, Series and Binomial Theorem | 4 | | | | |
| [ ] | KC `knowledge-check-01-06` | — | | | | |
| [ ] | KC `knowledge-check-07-12` | — | | | | |

## Precalculus 2e

`content/math/precalculus` · 12 chapters, 73 section pages, 2 knowledge checks

Never read against the September 22 bar: needs the full checker read AND
the image-first figure pass. The first math request is the pilot (README
step 0).

| | Chapter | Sections | Fixed | Errata | Commit | Notes |
|---|---|---|---|---|---|---|
| [ ] | 1. Functions | 7 | | | | |
| [ ] | 2. Linear Functions | 4 | | | | |
| [ ] | 3. Polynomial and Rational Functions | 9 | | | | |
| [ ] | 4. Exponential and Logarithmic Functions | 8 | | | | |
| [ ] | 5. Trigonometric Functions | 4 | | | | |
| [ ] | 6. Periodic Functions | 3 | | | | |
| [ ] | 7. Trigonometric Identities and Equations | 6 | | | | |
| [ ] | 8. Further Applications of Trigonometry | 8 | | | | |
| [ ] | 9. Systems of Equations and Inequalities | 8 | | | | |
| [ ] | 10. Analytic Geometry | 5 | | | | |
| [ ] | 11. Sequences, Probability and Counting Theory | 7 | | | | |
| [ ] | 12. Introduction to Calculus | 4 | | | | |
| [ ] | KC `knowledge-check-01-06` | — | | | | |
| [ ] | KC `knowledge-check-07-12` | — | | | | |

## Biology 2e

`content/life-health-sciences/biology` · 47 chapters, 208 section pages, 8 knowledge checks

Swept September 22–23, 2026 (figure-alt, longdesc, and claim passes all
done). Units bio01–06 ran the full-scope brief (now `brief-life-sciences.md`),
which is the standard: chapters 1–17 and 18.1 are done. The rest ran the
narrow brief, which skipped hints that state the fact without naming the
key, leaks not directly above, and footer counts. All eight knowledge
checks ran a narrow brief (they carry no hints; the gap is nearby leaks).

| | Chapter | Sections | Fixed | Errata | Commit | Notes |
|---|---|---|---|---|---|---|
| [x] | 1. The Study of Life | 2 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 2. The Chemical Foundation of Life | 3 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 3. Biological Macromolecules | 5 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 4. Cell Structure | 6 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 5. Structure and Function of Plasma Membranes | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 6. Metabolism | 5 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 7. Cellular Respiration | 7 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 8. Photosynthesis | 3 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 9. Cell Communication | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 10. Cell Reproduction | 5 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 11. Meiosis and Sexual Reproduction | 2 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 12. Mendel's Experiments and Heredity | 3 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 13. Modern Understandings of Inheritance | 2 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 14. DNA Structure and Function | 6 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 15. Genes and Proteins | 5 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 16. Gene Expression | 7 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 17. Biotechnology and Genomics | 5 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 18. Evolution and the Origin of Species | 3 | 29 | 983–984 | 71071e7 | 18.1 in the Sep 22 sweep; 18.2–18.3 Sep 23: one Opus fixer (~150k tokens) + Fable solve 7/7 (~60k); 15 of 29 were hint leaks |
| [x] | 19. The Evolution of Populations | 3 | 34 | 985–986 | 3250fc9 | Sep 23: one Opus fixer (~160k tokens) + Fable solve 3/3 (~45k); 22 of 34 were hint leaks; parent caught one fixer-made leak (a cloze stem stating the next MC's key) |
| [x] | 20. Phylogenies and the History of Life | 3 | 46 | 987 | — | Sep 23: one Opus fixer (~177k tokens) + Fable solve 8/8 (~53k); 27 of 46 were hint leaks; five reverse re-asks replaced with single-sentence clozes; parent turned the fixer's draft erratum (source "DNA" double-keys the phylogeny RQ) into an options deviation per errata 924/928/934 |
| [ ] | 21. Viruses | 4 | | | | |
| [ ] | 22. Prokaryotes: Bacteria and Archaea | 5 | | | | |
| [ ] | 23. Protists | 4 | | | | |
| [ ] | 24. Fungi | 5 | | | | |
| [ ] | 25. Seedless Plants | 4 | | | | |
| [ ] | 26. Seed Plants | 4 | | | | |
| [ ] | 27. Introduction to Animal Diversity | 4 | | | | |
| [ ] | 28. Invertebrates | 7 | | | | |
| [ ] | 29. Vertebrates | 7 | | | | |
| [ ] | 30. Plant Form and Physiology | 6 | | | | |
| [ ] | 31. Soil and Plant Nutrition | 3 | | | | |
| [ ] | 32. Plant Reproduction | 3 | | | | |
| [ ] | 33. The Animal Body: Basic Form and Function | 3 | | | | |
| [ ] | 34. Animal Nutrition and the Digestive System | 4 | | | | |
| [ ] | 35. The Nervous System | 5 | | | | |
| [ ] | 36. Sensory Systems | 5 | | | | |
| [ ] | 37. The Endocrine System | 5 | | | | |
| [ ] | 38. The Musculoskeletal System | 4 | | | | |
| [ ] | 39. The Respiratory System | 4 | | | | |
| [ ] | 40. The Circulatory System | 4 | | | | |
| [ ] | 41. Osmotic Regulation and Excretion | 5 | | | | |
| [ ] | 42. The Immune System | 4 | | | | |
| [ ] | 43. Animal Reproduction and Development | 7 | | | | |
| [ ] | 44. Ecology and the Biosphere | 5 | | | | |
| [ ] | 45. Population and Community Ecology | 7 | | | | |
| [ ] | 46. Ecosystems | 3 | | | | |
| [ ] | 47. Conservation Biology and Biodiversity | 4 | | | | |
| [ ] | KC `knowledge-check-01-03` | — | | | | |
| [ ] | KC `knowledge-check-04-10` | — | | | | |
| [ ] | KC `knowledge-check-11-17` | — | | | | |
| [ ] | KC `knowledge-check-18-20` | — | | | | |
| [ ] | KC `knowledge-check-21-29` | — | | | | |
| [ ] | KC `knowledge-check-30-32` | — | | | | |
| [ ] | KC `knowledge-check-33-43` | — | | | | |
| [ ] | KC `knowledge-check-44-47` | — | | | | |

## Microbiology

`content/life-health-sciences/microbiology` · 26 chapters, 127 section pages, 5 knowledge checks

Swept September 22–23, 2026 (figure-alt, longdesc, and claim passes all
done). Units mic01–08 ran the full-scope brief (now `brief-life-sciences.md`),
which is the standard: chapters 1–13 and 14.1–14.4 are done. The rest ran
the narrow brief, which skipped hints that state the fact without naming
the key, leaks not directly above, and footer counts. All five knowledge
checks ran a narrow brief (they carry no hints; the gap is nearby leaks).

| | Chapter | Sections | Fixed | Errata | Commit | Notes |
|---|---|---|---|---|---|---|
| [x] | 1. An Invisible World | 3 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 2. How We See the Invisible World | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 3. The Cell | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 4. Prokaryotic Diversity | 6 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 5. The Eukaryotes of Microbiology | 5 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 6. Acellular Pathogens | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 7. Microbial Biochemistry | 5 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 8. Microbial Metabolism | 7 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 9. Microbial Growth | 6 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 10. Biochemistry of the Genome | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 11. Mechanisms of Microbial Genetics | 7 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 12. Modern Applications of Microbial Genetics | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [x] | 13. Control of Microbial Growth | 4 | — | — | a145b91 | Sep 22 full-scope sweep |
| [~] | 14. Antimicrobial Drugs | 7 | | | | 14.1–14.4 done (mic08, full scope); resume at 14.5 |
| [ ] | 15. Microbial Mechanisms of Pathogenicity | 4 | | | | |
| [ ] | 16. Disease and Epidemiology | 4 | | | | |
| [ ] | 17. Innate Nonspecific Host Defenses | 5 | | | | |
| [ ] | 18. Adaptive Specific Host Defenses | 5 | | | | |
| [ ] | 19. Diseases of the Immune System | 5 | | | | |
| [ ] | 20. Laboratory Analysis of the Immune Response | 5 | | | | |
| [ ] | 21. Skin and Eye Infections | 5 | | | | |
| [ ] | 22. Respiratory System Infections | 4 | | | | |
| [ ] | 23. Urogenital System Infections | 6 | | | | |
| [ ] | 24. Digestive System Infections | 6 | | | | |
| [ ] | 25. Circulatory and Lymphatic System Infections | 4 | | | | |
| [ ] | 26. Nervous System Infections | 4 | | | | |
| [ ] | KC `knowledge-check-01-06` | — | | | | |
| [ ] | KC `knowledge-check-07-12` | — | | | | |
| [ ] | KC `knowledge-check-13-14` | — | | | | |
| [ ] | KC `knowledge-check-15-20` | — | | | | |
| [ ] | KC `knowledge-check-21-26` | — | | | | |
