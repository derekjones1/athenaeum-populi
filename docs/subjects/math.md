...
submission under the declared form. The class-by-class record moved to
...
The closed-class programme that made `answerForm` and its lint rules
necessary — the trivially satisfiable fill-in classes, their tokens, and the
extraction rules the programme was built on — moved to `docs/history/math.md`.
...
The corpus-wide replay audit that re-runs this record (`npm run
verify:replay` and its manual bare-RHS companion) moved to
`docs/history/math.md`.
