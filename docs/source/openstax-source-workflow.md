...
Biology is `in-progress`: its lock and pinned commit exist and chapters 1–42
are authored, so `build-map`/`verify-map` print it as `42/47 chapters, 182/208
...
- `data/openstax/source-map.json` (schema 2) connects all 456 authored
...
carries chapters 1–42 (182 sections). `npm run source:verify` and
`build-map` therefore print `biology: in-progress — 42/47 chapters, 182/208
