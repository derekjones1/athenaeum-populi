...
is `in-progress` and prints as `3/26 chapters, 11/127 sections mapped`.
...
- `data/openstax/source-map.json` (schema 2) connects all 493 authored
...
chapters 1–3 under `## Chapters` with the remaining twenty-three on its
"Planned contents" list (chapter 2, *How We See the Invisible World*, landed
the same day and chapter 3, *The Cell*, on September 6), and `verify-map` prints `3/26 chapters, 11/127 sections
mapped`.
