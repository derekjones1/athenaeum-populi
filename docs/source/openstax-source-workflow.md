...
Biology is `in-progress`: its lock and pinned commit exist and chapters 1–32
are authored, so `build-map`/`verify-map` print it as `32/47 chapters, 139/208
...
- `data/openstax/source-map.json` (schema 2) connects all 413 authored
...
carries chapters 1–32 (139 sections). `npm run source:verify` and
`build-map` therefore print `biology: in-progress — 32/47 chapters, 139/208
