...
Biology is `in-progress`: its lock and pinned commit exist and units 1–3 are
authored, so `build-map`/`verify-map` print it as `13/47 chapters, 51/208
...
- `data/openstax/source-map.json` (schema 2) connects all 325 authored
...
carries units 1–3 (chapters 1–13, 51 sections). `npm run source:verify` and
`build-map` therefore print `biology: in-progress — 13/47 chapters, 51/208
