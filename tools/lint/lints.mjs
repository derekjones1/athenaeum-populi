...
import { buildFigure, buildGraph, buildNumberLine } from '../../assets/js/lib/math/graph-core.mjs';
...
// tools/verify/verify-answers.mjs's DEFINITION_RE reads printed definitions — the
...
export function lintHugo(src, filename = '', options = {}) {
  ...
  // KaTeX-set math and which tools/verify/verify-replay.mjs cannot read as math
  ...
  for (const { params, inner, index, closed } of shortcodes(mediaSrc, 'mediafigure')) {
    ...
    // inside them as tag soup (39.4 shipped `P<sub>O₂</sub>` in an alt). Write
    // the plain form instead ("oxygen partial pressure (PO₂)"). A bare `>` is
    // fine — "MIC >32 µg/mL" is legitimate alt text, and the build audit
    // matches tags quote-aware (`openTagRe`) so it no longer truncates there.
