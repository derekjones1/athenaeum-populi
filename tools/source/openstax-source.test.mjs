...
test('committed provenance maps all 325 local sections exactly once', () => {
  const result = verifyCommittedSourceMap(repositoryRoot);
  assert.deepEqual(result.errors, []);
  assert.equal(result.expectedCount, 325);
  assert.equal(result.actualCount, 325);
  ...
  assert.equal(counts.biology.length, 51);
