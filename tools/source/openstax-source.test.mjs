...
test('committed provenance maps all 349 local sections exactly once', () => {
  const result = verifyCommittedSourceMap(repositoryRoot);
  assert.deepEqual(result.errors, []);
  assert.equal(result.expectedCount, 349);
  assert.equal(result.actualCount, 349);
  ...
  assert.equal(counts.biology.length, 75);
