...
- The lock pins one commit per upstream bundle: `prealgebra-bundle` for the
  ...
  parity is enforced book-wide. Biology is `in-progress`: chapters 1–23 (96
  sections — units 1–4, The Chemistry of Life through Evolutionary
  Processes, all complete, plus unit 5's first three chapters, Viruses
  through Protists) are authored under
  `content/life-health-sciences/biology`
  and the other 24 chapters have no landing yet, so `build-map`/`verify-map`
  print it as "23/47 chapters, 96/208 sections mapped" — visibly, never
...
- `npm run source:verify` — verify the committed 370-section map offline
