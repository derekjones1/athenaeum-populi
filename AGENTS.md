...
- The lock pins one commit per upstream bundle: `prealgebra-bundle` for the
  ...
  parity is enforced book-wide. Biology is `in-progress`: chapters 1–20 (83
  sections — units 1–4, The Chemistry of Life through Evolutionary
  Processes, all complete) are authored under
  `content/life-health-sciences/biology`
  and the other 27 chapters have no landing yet, so `build-map`/`verify-map`
  print it as "20/47 chapters, 83/208 sections mapped" — visibly, never
...
- `npm run source:verify` — verify the committed 357-section map offline
