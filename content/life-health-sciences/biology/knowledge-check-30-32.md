---
title: "Knowledge Check: Unit 6 — Chapters 30–32"
description: >-
  A cumulative self-test on Unit 6, Plant Structure and Function — plant
  form and physiology, soil and plant nutrition, and plant reproduction —
  with every question written from the chapters' own text in OpenStax
  Biology 2e.
source_chapters: "30-32"
weight: 38
---

{{< callout type="info" >}}
  **Test yourself on Unit 6: Plant Structure and Function (Chapters 30–32).** Every question below was written from
  the chapters' own text — their summaries, key terms, explanations, and
  tables — and none repeats a question from the section pages. There are no
  hints: treat it like a test. Questions are grouped by the section they
  come from, so a miss tells you exactly which section to review.
{{< /callout >}}

## Chapter 30: Plant Form and Physiology

### 30.1 The Plant Body

{{< textin
  question="Unlike the xylem's conducting cells, which are dead at functional maturity, the conducting cells of the ________ are alive at maturity."
  answer="phloem"
  accept="phloem tissue"
>}}

{{< textin
  question="Xylem tissue includes three different cell types: xylem parenchyma and two types that conduct water, vessel elements and ________."
  answer="tracheids"
  accept="tracheid"
>}}

{{< selfcheck question="How does permanent tissue form from meristematic tissue, and what are its three main types?" >}}
Meristems produce cells that quickly differentiate, or specialize, and become permanent tissue, taking on specific roles and losing their ability to divide further. They differentiate into three main types: dermal, vascular, and ground tissue.
===CHECKS===
meristems produce cells that quickly differentiate, or specialize, and become permanent tissue
taking on specific roles and losing their ability to divide further
they differentiate into three main types: dermal, vascular, and ground tissue
{{< /selfcheck >}}

### 30.2 Stems

{{< textin
  question="A tough, waterproof outer layer of cork cells that further protects a woody plant from damage is called ________."
  answer="bark"
>}}

{{< multiplechoice
  question="Which xylem cell type has thick, lignified secondary cell walls, with water moving from cell to cell only through pits where the secondary wall is absent?"
  answer="tracheid"
>}}
vessel element
sieve-tube cell
companion cell
tracheid
{{< /multiplechoice >}}

{{< selfcheck question="How do stolons differ from runners, and what is a familiar example of a plant that spreads by runners?" >}}
Stolons are stems that run almost parallel to the ground, or just below the surface, and can give rise to new plants at the nodes. Runners are a type of stolon that runs above the ground and produces new clone plants at nodes at varying intervals; strawberries are an example.
===CHECKS===
stolons are stems that run almost parallel to the ground, or just below the surface
runners are a type of stolon that runs above the ground and produces new clone plants at nodes
strawberries are an example
{{< /selfcheck >}}

### 30.3 Roots

{{< textin
  question="The region of root ground tissue that lies between the epidermis and the vascular tissue is called the ________."
  answer="cortex"
>}}

{{< multiplechoice
  question="In monocot roots, how is the vascular tissue of the stele arranged?"
  answer="in a ring around the pith"
>}}
in a ring around the pith
in an X shape at the root's center
outside the endodermis, surrounding the pericycle
{{< /multiplechoice >}}

{{< selfcheck question="What are the three major functions of the roots of seed plants?" >}}
The roots of seed plants anchor the plant to the soil, absorb water and minerals and transport them upwards, and store the products of photosynthesis.
===CHECKS===
anchor the plant to the soil
absorb water and minerals and transport them upwards
store the products of photosynthesis
{{< /selfcheck >}}

### 30.4 Leaves

{{< multiplechoice
  question="In which leaf arrangement do two leaves arise at the same point along the branch?"
  answer="opposite"
>}}
alternate
spiral
opposite
whorled
{{< /multiplechoice >}}

{{< textin
  question="Botanists call the upper surface of a leaf the ________ surface."
  answer="adaxial"
  accept="adaxis"
>}}

{{< selfcheck question="How are the leaves of many aquatic plants adapted to living on the water's surface?" >}}
Many aquatic plants have leaves with a wide lamina that can float on the surface of the water, and a thick waxy cuticle on the leaf surface that repels water.
===CHECKS===
leaves with a wide lamina that can float on the surface of the water
a thick waxy cuticle on the leaf surface that repels water
{{< /selfcheck >}}

### 30.5 Transport of Water and Solutes in Plants

{{< multiplechoice
  question="Which water-potential component becomes more influential the taller a plant grows, adding roughly 10 MPa of extra resistance to overcome in a tall redwood?"
  answer="gravity potential"
>}}
pressure potential
gravity potential
solute potential
matric potential
{{< /multiplechoice >}}

{{< textin
  question="The formation of small gas bubbles in the xylem, which can interrupt the continuous stream of water moving up the plant, is called ________."
  answer="cavitation"
>}}

{{< selfcheck question="How is sucrose loaded into the phloem's sieve-tube elements at the source, and why does this step require ATP?" >}}
The sucrose is actively transported against its concentration gradient, a process requiring ATP, into the phloem cells using the electrochemical potential of the proton gradient. This is coupled to the uptake of sucrose with a carrier protein called the sucrose-H⁺ symporter.
===CHECKS===
the sucrose is actively transported against its concentration gradient
a process requiring ATP
using the electrochemical potential of the proton gradient
coupled to the uptake of sucrose with a carrier protein called the sucrose-H⁺ symporter
{{< /selfcheck >}}

### 30.6 Plant Sensory Systems and Responses

{{< textin
  question="The physiological process that leads to the fall of a plant organ, such as a leaf or petal, is called ________."
  answer="abscission"
>}}

{{< multiplechoice
  question="Which term describes a plant's ability to use light to track time, such as sensing day length to anticipate the season?"
  answer="photoperiodism"
>}}
photomorphogenesis
phototropism
phytochrome
photoperiodism
{{< /multiplechoice >}}

{{< selfcheck question="How do jasmonates help a damaged plant defend itself, including their role in attracting other insects?" >}}
As tissue is damaged, jasmonates may promote the synthesis of compounds that are toxic to predators. Jasmonates also elicit the synthesis of volatile compounds that attract parasitoids, which are insects that spend their developing stages in or on another insect and eventually kill their host.
===CHECKS===
jasmonates promote the synthesis of compounds that are toxic to predators
jasmonates elicit the synthesis of volatile compounds that attract parasitoids
parasitoids are insects that spend their developing stages in or on another insect and eventually kill their host
{{< /selfcheck >}}

## Chapter 31: Soil and Plant Nutrition

### 31.1 Nutritional Requirements of Plants

{{< multiplechoice
  question="Which two factors are the major determinants of plant distribution and growth?"
  answer="soil quality and climate"
>}}
water and carbon dioxide
soil quality and climate
carbon and potassium
macronutrients and micronutrients
{{< /multiplechoice >}}

{{< selfcheck question="What roles does phosphorus play in a plant, both in synthesizing molecules and in energy metabolism?" >}}
Phosphorus is necessary to synthesize nucleic acids and phospholipids. As part of ATP, phosphorus enables food energy to be converted into chemical energy through oxidative phosphorylation.
===CHECKS===
phosphorus is necessary to synthesize nucleic acids and phospholipids
phosphorus enables food energy to be converted into chemical energy through oxidative phosphorylation
{{< /selfcheck >}}

{{< textin
  question="The combination of ________, along with water, carbon dioxide, and sunlight, produce the energy that allows plants to grow."
  answer="organic compounds"
  accept="organic compound|organic molecules|organic molecule"
>}}

### 31.2 The Soil

{{< multiplechoice
  question="About what percentage of soil volume is organic matter?"
  answer="5 percent"
>}}
45 percent
50 percent
25 percent
5 percent
{{< /multiplechoice >}}

{{< textin
  question="The area of soil affected by root secretions and microorganisms is called the ________."
  answer="rhizosphere"
  accept="rhizospheres"
>}}

{{< selfcheck question="Why do sediments carried by a fast-moving river differ from those carried by a slow-moving river?" >}}
A fast-moving river could have sediments of rocks and sand, whereas a slow-moving river could have fine-textured material, such as clay.
===CHECKS===
a fast-moving river could have sediments of rocks and sand
a slow-moving river could have fine-textured material, such as clay
{{< /selfcheck >}}

### 31.3 Nutritional Adaptations of Plants

{{< multiplechoice
  question="Which term describes a parasitic plant that is fully photosynthetic and uses its host only for water and minerals?"
  answer="hemiparasite"
>}}
hemiparasite
holoparasite
saprophyte
epiphyte
{{< /multiplechoice >}}

{{< multiplechoice
  question="In a mycorrhizal partnership, what percentage of the total carbon accessed by the plant can the fungus obtain?"
  answer="up to 20 percent"
>}}
up to 20 percent
more than 80 percent
65 percent
{{< /multiplechoice >}}

{{< selfcheck question="Why can plants not take advantage of atmospheric nitrogen (N₂) directly?" >}}
Plants cannot take advantage of atmospheric nitrogen because they do not have the necessary enzymes to convert it into biologically useful forms.
===CHECKS===
plants cannot take advantage of atmospheric nitrogen
they do not have the necessary enzymes to convert it into biologically useful forms
{{< /selfcheck >}}

## Chapter 32: Plant Reproduction

### 32.1 Reproductive Development and Structure

{{< textin
  question="The thicker, outer covering of a pollen grain, which contains the waterproofing substance sporopollenin, is called the ________."
  answer="exine"
  accept="exines"
>}}

{{< multiplechoice
  question="In the angiosperm embryo sac, the three nuclei that position themselves at the end opposite the micropyle and later degenerate develop into the ________."
  answer="antipodal cells"
>}}
egg cell
central cell
antipodal cells
synergid cells
{{< /multiplechoice >}}

{{< selfcheck question="Explain how the location of the female gametophyte differs between angiosperms and gymnosperms, and name two other ways gymnosperm reproduction differs from that of angiosperms." >}}
In angiosperms, the female gametophyte exists in an enclosed structure—the ovule—which is within the ovary; in gymnosperms, the female gametophyte is present on exposed bracts of the female cone. Double fertilization is a key event in the lifecycle of angiosperms, but is completely absent in gymnosperms. The male and female gametophyte structures are present on separate male and female cones in gymnosperms, whereas in angiosperms, they are a part of the flower.
===CHECKS===
in angiosperms, the female gametophyte exists in an enclosed structure—the ovule—which is within the ovary
in gymnosperms, the female gametophyte is present on exposed bracts of the female cone
double fertilization is a key event in the lifecycle of angiosperms, but is completely absent in gymnosperms
the male and female gametophyte structures are present on separate male and female cones in gymnosperms, whereas in angiosperms, they are a part of the flower
{{< /selfcheck >}}

### 32.2 Pollination and Fertilization

{{< textin
  question="The cold treatment that some seeds require before germination, guaranteeing that seeds from plants in temperate climates will not germinate until spring, is called ________."
  answer="vernalization"
  accept="vernalisation"
>}}

{{< multiplechoice
  question="In a germinating monocot seed, which structure secretes the enzymes that degrade the endosperm's stored carbohydrates, proteins, and lipids?"
  answer="the aleurone"
>}}
the scutellum
the aleurone
the testa
the endosperm
{{< /multiplechoice >}}

{{< selfcheck question="Explain how simple, aggregate, and multiple fruits differ from each other in how many carpels and flowers each develops from." >}}
If the fruit develops from a single carpel or fused carpels of a single ovary, it is known as a simple fruit, as seen in nuts and beans. An aggregate fruit is one that develops from more than one carpel, but all are in the same flower: the mature carpels fuse together to form the entire fruit, as seen in the raspberry. Multiple fruit develops from an inflorescence or a cluster of flowers. An example is the pineapple, where the flowers fuse together to form the fruit.
===CHECKS===
if the fruit develops from a single carpel or fused carpels of a single ovary, it is known as a simple fruit
an aggregate fruit is one that develops from more than one carpel, but all are in the same flower
multiple fruit develops from an inflorescence or a cluster of flowers
an example is the pineapple, where the flowers fuse together to form the fruit
{{< /selfcheck >}}

### 32.3 Asexual Reproduction

{{< textin
  question="The artificial method of asexual reproduction in which a stem still attached to the plant is bent and covered with soil, used to propagate jasmine and bougainvillea, is called ________."
  answer="layering"
>}}

{{< multiplechoice
  question="In *Bryophyllum* and kalanchoe, small buds on the margins of the ________ can detach and grow into independent plants."
  answer="leaves"
>}}
stems
roots
bulbs
leaves
{{< /multiplechoice >}}

{{< selfcheck question="Explain how grafting joins two plants together, from the initial cut through to the graft bearing flowers and fruit." >}}
In grafting, two plant species are used; part of the stem of the desirable plant is grafted onto a rooted plant called the stock. The part that is grafted or attached is called the scion. Both are cut at an oblique angle and placed in close contact with each other, and are then held together. The vascular systems of the two plants grow and fuse, forming a graft. After a period of time, the scion starts producing shoots, and eventually starts bearing flowers and fruits.
===CHECKS===
part of the stem of the desirable plant is grafted onto a rooted plant called the stock
the part that is grafted or attached is called the scion
both are cut at an oblique angle and placed in close contact with each other
the vascular systems of the two plants grow and fuse, forming a graft
after a period of time, the scion starts producing shoots, and eventually starts bearing flowers and fruits
{{< /selfcheck >}}

---

<small>This knowledge check is written for Unit 6 (Chapters 30–32) of [Biology 2e](https://openstax.org/books/biology-2e/pages/30-introduction) by Mary Ann Clark, Jung Choi, Matthew Douglas, and OpenStax, © OpenStax, licensed under [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/). Access the original for free at [openstax.org](https://openstax.org/details/books/biology-2e). Changes: every question is locally written from the pinned text of the chapters' own modules — their section summaries, glossary definitions, body passages, and comparison tables — and none is transcribed from the book's exercise sets, which the section pages already carry; no question carries a hint, and every self-check carries a rubric that decomposes a model answer made of the module's own sentences.</small>
