...
description: >-
  How scientists distinguish homologous from analogous traits using
  morphology and molecular data, how cladistics groups organisms into clades
  descended from a shared ancestor, and how maximum parsimony favors the
  simplest evolutionary pathway that fits the evidence — adapted from
  OpenStax Biology 2e, Section 20.2.
source_section: "20.2"
weight: 2
---
...
{{< mediafigure src="biology/Figure_20_02_01-5b8b" kind="diagram" eager="true" alt="Two panels: (a) an osprey flying against blue sky, beside a white line-drawing of its wing's bone structure — one long upper bone, a bend to a pair of longer bones, then jointed finger bones; (b) a bat in flight against a dark background, beside a similar line-drawing of its wing skeleton, whose finger bones are much longer and more separated, forming a scaffold for the wing membrane." >}}
...
{{< mediafigure src="biology/Figure_20_02_02-6526" kind="diagram" alt="Three panels: (a) a bat in flight, beside a line-drawing of its long, splayed finger-bone wing skeleton; (b) an osprey in flight, beside a line-drawing of its shorter, fused wing-bone skeleton; (c) a honeybee on a flower, beside a line-drawing of its wing's branching vein pattern, which has no bones at all." >}}
...
{{< callout type="info" >}}
**Link to Learning.** Explore [a site with examples of how appearances can be misleading when interpreting phylogenetic relationships](https://openstax.org/l/relationships).
{{< /callout >}}
...
{{< mediafigure src="biology/Figure_20_02_03-eb43" kind="diagram" alt="A detailed botanical engraving of a Dalbergia sissoo branch, showing pinnately compound leaves with rows of oval leaflets, small flower buds clustered at the branch tip, and several long, flat seed pods hanging below the leaves, with separate close-up sketches of an individual pod and flower parts at lower right." >}}
...
the amniotic egg evolved before hair
the amniota clade is larger than the clade that encompasses animals with hair
{{< /selfcheck >}}

Clades can vary in size depending on which branch point one references. The important factor is that all organisms in the clade or monophyletic group stem from a single point on the tree. You can remember this because monophyletic breaks down into “mono,” meaning one, and “phyletic,” meaning evolutionary relationship. The figure below shows various clade examples. Notice how each clade comes from a single point; whereas, the non-clade groups show branches that do not share a single point.

{{< mediafigure src="biology/Figure_20_02_05" kind="diagram" alt="Four small trees in a 2×2 grid, all with the same shape and the same ten tip labels (Entamoebae, Slime molds, Animals, Fungi, Plants, Ciliates, Flagellates, Trichomonads, Microsporidia, Diplomonads), grouped 'Clades' on top and 'Not Clades' on the bottom; a shaded patch in each tree highlights a different subset of tips to show which subsets share a single branch point and which do not." longdesc="All four panels share one tree shape: a single trunk runs from a root at the lower left up to the upper right, and along it, nearest the root first, branch points lead off in order to Diplomonads, Microsporidia, Trichomonads, Flagellates, Entamoebae, Slime molds, and Ciliates, before the trunk ends in one final branch point that splits three ways into Animals, Fungi, and Plants. Top-left panel ('Clades'): Animals, Fungi, and Plants, which share that final branch point, are all shaded as one clade. Top-right panel ('Clades'): only Flagellates is shaded, shown as a clade of one. Bottom-left panel ('Not Clades'): Flagellates and Ciliates are shaded together even though they branch off the trunk at different points. Bottom-right panel ('Not Clades'): Animals and Plants are shaded together while Fungi, which shares their branch point, is left unshaded." >}}
...
{{< callout type="info" >}}
**Link to Learning.** Visit [a site explaining how researchers use maximum parsimony to build phylogenetic trees](https://openstax.org/l/using_parsimony).
...
their evolutionary paths are quite separate
...
{{< multiplechoice
  ...
  hint="Cladistics sorts organisms based on features that stem from a shared evolutionary past, not independently evolved ones."
>}}
homologous traits
homoplasies
analogous traits
...
{{< multiplechoice
  ...
  answer="They evolved from a shared ancestor."
  hint="A clade is defined by descent, not by how similar the organisms look today."
>}}
...
{{< selfcheck question="Why is it so important for scientists to distinguish between homologous and analogous characteristics before building phylogenetic trees?" hint="Think about what happens to every later branch point built on top of one wrong connection." >}}
...
if an analogous similarity were used on a tree, this would be erroneous
would cause the subsequent branches to be inaccurate
{{< /selfcheck >}}

{{< textin
  question="The system used to organize homologous traits in order to describe phylogenies is called ________."
  answer="cladistics"
  hint="It sorts organisms into clades — groups that descended from a single ancestor."
>}}
...
{{< textin
  question="The concept of explaining evolutionary events in the simplest, most obvious way, using the fewest possible steps, is called ________."
  answer="maximum parsimony"
  hint="Think of a group of hikers following established trails rather than forging new ones."
>}}
