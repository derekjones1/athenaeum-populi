...
description: >-
  How prokaryotic structural genes are organized into operons under a single
  promoter and controlled by repressors, activators, and inducers, using the
  repressible *trp* operon and the inducible, CAP-activated *lac* operon as
  models, plus higher-level and prokaryote-specific regulatory mechanisms —
  adapted from OpenStax Microbiology, Section 11.7.
source_section: "11.7"
weight: 7
---
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_Operon" alt="A labelled diagram of a generic operon. At the left, a regulatory gene sits upstream of the operon. The operon itself is bracketed as promoter, then operator, then four structural genes labelled A, B, C, and D in a row. An arrow marked transcription leads from the operon to a single mRNA strand spanning all four structural genes. Four arrows marked translation lead from the mRNA to four separate circles labelled protein A, protein B, protein C, and protein D." longdesc="Reading left to right: a DNA strand opens with a short unlabelled segment, then a two-part block bracketed regulatory gene, then a wider unlabelled spacer, then the operon proper, bracketed overall as operon and broken into a promoter block, an operator block, and four equal blocks labelled A, B, C, and D in order under a bracket labelled structural genes; a block labelled terminator follows, just outside the operon bracket. A downward arrow labelled transcription leads from the operon to a single horizontal bar labelled mRNA that spans the same length as the four structural genes. From that one mRNA bar, four downward arrows labelled translation lead to four separate circles labelled protein A, protein B, protein C, and protein D, showing that one polycistronic transcript is translated into four separate proteins." kind="diagram" eager="true" >}}
...
{{< selfcheck question="What are the parts in the DNA sequence of an operon?" hint="Reread the first two paragraphs of Prokaryotic Gene Regulation and the operon figure's caption." >}}
...
===CHECKS===
includes a promoter
within the regulatory region ... an operator
the structural genes of the operon, which are transcribed together as a single polycistronic transcript
{{< /selfcheck >}}

{{< selfcheck question="What types of regulatory molecules are there?" hint="Reread the second paragraph of Prokaryotic Gene Regulation." >}}
...
===CHECKS===
A repressor is a transcription factor that suppresses transcription of a gene by binding to the operator
An activator is a transcription factor that increases the transcription of a gene by facilitating RNA polymerase binding to the promoter
An inducer is a small molecule that either activates or represses transcription by interacting with a repressor or an activator
{{< /selfcheck >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_trp" alt="A two-panel diagram of the trp operon. The top panel shows RNA polymerase bound to the promoter with an arrow running rightward beneath the operator and trpE, its head partway under trpD, and a separate, unbound repressor shape below. The bottom panel shows the same gene row but with the repressor now sitting on the operator, a small tryptophan shape attached to it, and a red X blocking the arrow into the operon." longdesc="Two stacked panels, each showing the same row of blocks: promoter, operator, trpE, trpD, trpC, trpB, trpA, left to right. Top panel (absence of tryptophan): an oval labelled RNA polymerase sits on the promoter with a rightward arrow running beneath the row from the promoter's end past the operator and trpE, its head partway under trpD (it does not reach trpC, trpB, or trpA), showing transcription proceeding; a separate rounded shape labelled repressor sits below the gene row, unattached to anything. Bottom panel (tryptophan present): the same RNA polymerase oval sits on the promoter, but a red X sits on the arrow just past the promoter; the repressor shape now sits directly on the operator, and a small pentagon labelled tryptophan is attached to the repressor, showing that tryptophan-bound repressor blocks RNA polymerase from moving into the operon." kind="diagram" >}}
...
Watch this [video](https://openstax.org/l/22trpoperon) to learn more about the *trp* operon.
{{< /callout >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_lacrep" alt="A two-panel diagram of the lac operon. The top panel shows a repressor shape sitting on the operator with a red X blocking the arrow from RNA polymerase just past the promoter. The bottom panel shows the repressor detached from the operator with a small lactose circle attached to it, and an unblocked arrow running from RNA polymerase, its head partway under the operator." longdesc="Two stacked panels, each showing the same row of blocks: promoter, operator, lacZ, lacY, lacA, left to right. Top panel (lactose absent): an oval labelled RNA polymerase sits on the promoter; a rounded shape labelled repressor sits on the operator, and a red X blocks the arrow just past the promoter, before it reaches the operator. Bottom panel (lactose present): the repressor shape now sits apart from the operator with a small circle labelled lactose attached to it, and an unbroken arrow runs rightward from RNA polymerase, its head partway under the operator (the drawing does not extend it under lacZ, lacY, or lacA), showing transcription proceeding. Printed above the panels: 'In the absence of lactose, the lac repressor binds the operator, and transcription is blocked.' and 'In the presence of lactose, the lac repressor is released from the operator, and transcription proceeds at a slow rate.'" kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_Diaux" alt="A diauxic growth curve: the log of E. coli cells (y-axis, 0 to 2.0) is plotted against time in hours (x-axis, 0 to about 10.5). The curve rises from about 0.15 through a first steep climb to about 1.1 by hour 4.5, levels off for about an hour, then climbs a second time to about 1.9 by hour 9 before flattening again. The first climb is labelled E. coli uses glucose; the second climb is labelled E. coli uses lactose." longdesc="Reading left to right: the curve begins near 0.15 at time 0 and rises only gently for the first two hours, to about 0.35. It then climbs steeply to about 1.1 at hour 4.5, a segment labelled E. coli uses glucose. The curve levels off at about 1.1 from hour 4.5 to about hour 5.5, then climbs a second time, less steeply, to about 1.85 at hour 9, a segment labelled E. coli uses lactose, before flattening near 1.9 for the rest of the plotted time." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_CAP" alt="A two-panel diagram, (a) and (b), of the lac operon promoter with and without cAMP. Panel (a): without cAMP, a CAP oval floats free of the promoter and a thin arrow shows RNA polymerase transcribing at a low rate; with cAMP, one oval printed 'cAMP + CAP' sits on the promoter and a thick arrow shows faster transcription. Panel (b): with the same cAMP + CAP oval on the promoter and no repressor, RNA polymerase transcribes with the same thick arrow; with the complex present but a repressor also bound to the operator, a red X blocks the short arrow from RNA polymerase toward the repressor." longdesc="Panel (a), two rows over the same promoter-operator-lacZ-lacY-lacA gene row. Top row: a separate oval labelled CAP floats apart from the promoter, RNA polymerase sits on the promoter, and a thin arrow runs rightward beneath the operator, showing transcription at a low rate. Bottom row: a single oval printed 'cAMP + CAP' now sits directly on the promoter, RNA polymerase sits beside it, and a thick orange arrow runs rightward beneath the operator, showing faster transcription. Panel (b), two rows over the same gene row: top row, the same single 'cAMP + CAP' oval sits on the promoter beside RNA polymerase, no repressor is present, and the same thick arrow runs beneath the operator, showing that the complex stimulates RNA polymerase activity and increases RNA synthesis. Bottom row: the same 'cAMP + CAP' oval sits on the promoter beside RNA polymerase, but a separate repressor shape now sits on the operator, and a red X blocks the thin arrow from RNA polymerase toward the repressor, showing that repressor binding still blocks transcription even with the cAMP + CAP complex present." kind="diagram" >}}
...
Watch an [animated tutorial](https://openstax.org/l/22lacoperon) about the workings of the *lac* operon here.
{{< /callout >}}

**Check Your Understanding**

{{< multiplechoice
  question="What affects the binding of the *trp* operon repressor to the operator?"
  answer="Tryptophan binds to the repressor, changing its shape so it can bind the operator."
  hint="Reread the second paragraph of The trp Operon subsection."
>}}
Tryptophan binds to the repressor, changing its shape so it can bind the operator.
Lactose binds to the repressor, changing its shape so it can bind the operator.
cAMP binds to catabolite activator protein, allowing it to bind the promoter.
An inducer binds directly to the operator, blocking RNA polymerase.
{{< /multiplechoice >}}

{{< selfcheck question="How and when is the behavior of the *lac* repressor protein altered?" hint="Reread the second paragraph of The lac Operon: An Inducible Operon." >}}
...
the lactose is converted to allolactose
allolactose ... binds to the repressor and changes its shape so that it is no longer able to bind the operator DNA
allows RNA polymerase to transcribe the *lac* structural genes
{{< /selfcheck >}}

{{< multiplechoice
  question="In addition to being repressible, how else is the *lac* operon regulated?"
  answer="It is also subject to activation, in the absence of glucose."
  hint="Reread the first sentence of The lac Operon: An Inducible Operon."
>}}
It is also subject to activation, in the absence of glucose.
It is also regulated by attenuation.
It is also regulated by a riboswitch.
It is also regulated by production of alarmones.
{{< /multiplechoice >}}
...
{{< textin
  question="What is the name given to a collection of operons that can be regulated as a group?"
  answer="regulon"
  hint="Reread the Global Responses of Prokaryotes paragraph."
>}}

{{< textin
  question="What type of stimulus would trigger the transcription of a different σ factor?"
  answer="environmental conditions"
  accept="specific environmental conditions|environmental stress|stress|environmental change|a change in the environment|environmental signal"
  hint="Reread the middle of the Alternate σ Factors paragraph."
>}}
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_Atten" alt="A diagram of trp attenuation showing a shared initial state — a ribosome sitting at the AUG start codon, with mRNA regions 1 and 2 just past it paired into a stem-loop, while RNA polymerase pauses on the trpL DNA below — that branches into two outcomes: (a) High level of tryptophan, where the ribosome completes translation, regions 3 and 4 form a terminator stem loop, and RNA polymerase is blocked and detaches; and (b) Low level of tryptophan, where the ribosome stalls at region 1, regions 2 and 3 form an antiterminator stem loop, and RNA polymerase proceeds into the trp-regulated genes." longdesc="At the left, a shared initial state: a ribosome sits at the AUG start codon near the mRNA's 5′ end, and just downstream of it regions 1 and 2 have already paired into a small stem-loop; below, on the DNA, RNA polymerase is shown pausing as it transcribes the trpL leader sequence. Two grey arrows lead right from this shared state. The upper arrow, labelled termination conformation, leads to panel (a), High level of tryptophan: the ribosome has proceeded across regions 1 and 2 to produce a complete polypeptide (a chain of circles) and the label Ribosome proceeds; regions 3 and 4 now pair into a terminator stem loop; RNA polymerase, blocked by a red X just past a run of U's at the mRNA's 3′ end, is shown detaching, labelled Transcription terminates. The lower arrow, labelled readthrough conformation, leads to panel (b), Low level of tryptophan: the ribosome stalls, blocked by a red X, after producing only an incomplete polypeptide from region 1, labelled Ribosome stalls; regions 2 and 3 pair into an antiterminator stem loop instead, leaving region 4 an unpaired stretch of mRNA ahead of the trp-regulated genes; RNA polymerase proceeds rightward past it, labelled Transcription proceeds." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_Riboswitch" alt="A two-panel diagram of a riboswitch, each panel showing an mRNA strand with an on state above and an off state below. Panel (a): an antiterminator stem loop lets RNA polymerase proceed (on); a small molecule binds the riboswitch, shifting the mRNA into a terminator loop that blocks it (off). Panel (b): the riboswitch is followed by an open ribosome binding site where translation proceeds (on); the same binding folds that site into a blocked loop (off)." longdesc="Panel (a): top row, an mRNA strand folds into a small riboswitch loop followed by a larger antiterminator stem loop; RNA polymerase sits on a DNA line to the right with an arrow proceeding rightward, the printed words Transcription proceeds, and the label on. Bottom row, the same mRNA after a small molecule (drawn as an irregular shape) has bound the riboswitch loop: the second loop is now shown as a terminator stem loop, a red X blocks the arrow, and the label reads off. Panel (b) mirrors this layout for translation: top row, the mRNA's riboswitch loop is followed by a second, unlabelled stem loop and then an open stretch labelled ribosome binding site, where a ribosome sits translating a chain of circles labelled polypeptide, with the printed words Translation proceeds and the label on. Bottom row, after the small molecule binds the riboswitch, the region that was the ribosome binding site is now folded into its own stem loop labelled blocked ribosome binding site, a red X blocks the arrow, and the label reads off." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_11_07_Enhancer" alt="Two diagrams of looped DNA, one for gene A (left) and one for gene B (right). In each, a purple DNA bending protein bends the DNA so that an enhancer, a yellow stretch carrying three distal control elements, lies just above the promoter; an activator bound to each element rests on a cluster of transcription factors and mediator proteins sitting over the promoter, with RNA polymerase beneath it. The two genes share the same promoter and protein cluster, but two of gene B's three distal control elements and activators differ in colour from gene A's." longdesc="Left diagram (gene A): a light-blue DNA strand runs left to right, bent back on itself into a loop at its left end, where a purple crescent labelled DNA bending protein sits inside the bend. The upper arm of the loop carries a yellow stretch bracketed enhancer, with three coloured segments on it (blue, light blue, green) labelled distal control elements. A small square of matching colour, labelled activators, hangs below each segment and rests on a cluster of overlapping ovals (red, lavender, pale green, purple, green, blue, and a large salmon oval) labelled transcription factors and mediator proteins. The cluster sits over the lower arm's promoter block, which lies immediately left of a green block labelled gene A; a pale oval below the promoter is labelled RNA polymerase. Right diagram (gene B): the same layout, unlabelled except for gene B, with the same promoter, the same protein cluster, and RNA polymerase, but its three distal control elements and their activators are purple, light blue, and red — two of the three differ from gene A's." kind="diagram" >}}
...
This [video](https://openstax.org/l/22epigreg) describes how epigenetic regulation controls gene expression.
{{< /callout >}}

**Check Your Understanding**

{{< selfcheck question="What stops or allows transcription to proceed when attenuation is operating?" hint="Reread the attenuation paragraph of Additional Methods of Regulation in Bacteria." >}}
...
===CHECKS===
When a terminator stem-loop forms ... transcription terminates, releasing RNA polymerase from the mRNA
When an antiterminator stem-loop forms instead, this prevents the formation of the terminator stem-loop
RNA polymerase can continue and transcribe the structural genes
{{< /selfcheck >}}

{{< textin
  question="What determines the state of a riboswitch?"
  answer="a small molecule"
  accept="small intracellular molecule|binding of a small intracellular molecule|binding of a small molecule"
  hint="Reread the riboswitch paragraph of Additional Methods of Regulation in Bacteria."
>}}

**Check Your Understanding**

{{< multiplechoice
  question="Describe the function of an enhancer."
  answer="It is a DNA sequence that promotes transcription."
  hint="Recall the one-sentence definition of an enhancer given in its figure caption."
>}}
It is a DNA sequence that promotes transcription.
It is a DNA sequence to which a repressor binds to block transcription.
It is a transcription factor that increases the transcription of a gene by facilitating RNA polymerase binding to the promoter.
It is a small molecule that either activates or represses transcription by interacting with a repressor or an activator.
{{< /multiplechoice >}}

{{< selfcheck question="Describe two mechanisms of epigenetic regulation in eukaryotes." hint="Reread the DNA-Level Control subsection." >}}
...
===CHECKS===
methylation of certain cytosine nucleotides in DNA ... commonly correlat[es] to lowered levels of gene expression
chemical modification of histone proteins, including acetylation and deacetylation
influences the packaging state of DNA and thus affect[s] the availability of loosely wound DNA for transcription
{{< /selfcheck >}}
...
This is the final installment of Mark's case; it began in [The Functions of Genetic Material](/life-health-sciences/microbiology/11-mechanisms-of-microbial-genetics/01-the-functions-of-genetic-material/).
{{< /callout >}}
...
{{< multiplechoice
  question="An operon of genes encoding enzymes in a biosynthetic pathway is likely to be which of the following?"
  answer="repressible"
  hint="Reread the Regulation by Repression paragraph."
>}}
inducible
repressible
constitutive
...
{{< multiplechoice
  question="An operon encoding genes that are transcribed and translated continuously to provide the cell with constant intermediate levels of the protein products is said to be which of the following?"
  answer="constitutive"
  hint="Reread the third paragraph of Prokaryotic Gene Regulation."
>}}
repressible
inducible
constitutive
...
{{< multiplechoice
  question="Which of the following conditions leads to maximal expression of the *lac* operon?"
  answer="lactose present, glucose absent"
  hint="Reread the third paragraph of the subsection on activation of the lac operon."
>}}
...
{{< textin
  question="The DNA sequence, to which repressors may bind, that lies between the promoter and the first structural gene is called the ________."
  answer="operator"
  hint="Reread the second paragraph of Prokaryotic Gene Regulation."
>}}

{{< mediafigure src="microbiology/OSC_Microbio_11_07_Diaux_img" alt="A growth curve graph with time in hours on the x-axis and density of bacteria as a percentage on the y-axis. An upward-sloping segment early in the curve is labelled A. It is followed by a plateau labelled B. A second upward-sloping segment follows, labelled C, ending in a final plateau labelled D." kind="diagram" >}}
...
{{< selfcheck question="The following figure is from Monod's original work on diauxic growth showing the growth of *E. coli* in the simultaneous presence of xylose and glucose as the only carbon sources. Explain what is happening at points A–D with respect to the carbon source being used for growth, and explain whether the xylose-use operon is being expressed (and why). Note that expression of the enzymes required for xylose use is regulated in a manner similar to the expression of the enzymes required for lactose use." hint="Work each point from the diauxic-growth paragraph and the lac operon subsections, treating xylose as the question's note instructs." >}}
...
===CHECKS===
At point A, the preferred substrate glucose is being used for growth, and *E. coli* grows quickly
the xylose-use operon is not expressed
At point B, growth rates slow because glucose has become depleted
cAMP levels begin to rise, and the accumulating cAMP-CAP complex can begin activating the promoter
At point C, growth resumes ... as the enzymes needed for the metabolism of xylose ... are expressed
the xylose-use operon is expressed at this point because xylose is present (removing the operon's repressor) and glucose is depleted (allowing the CAP-cAMP complex to activate the promoter)
{{< /selfcheck >}}

{{< sortbins question="Sort each condition described below by the transcription outcome it produces in the lac operon, from the table above." hint="Match each condition pair against the Glucose/Lactose or CAP/Repressor columns of the table above, then read off its Transcription column." >}}
{"bins": ["No", "Some", "Yes"],
 "items": [
  {"label": "Glucose present, lactose absent", "bin": 0},
  {"label": "Glucose present, lactose present", "bin": 1},
  {"label": "Glucose absent, lactose absent", "bin": 0},
  {"label": "Glucose absent, lactose present", "bin": 2},
  {"label": "CAP not bound, repressor bound", "bin": 0},
  {"label": "CAP not bound, repressor not bound", "bin": 1},
  {"label": "CAP bound, repressor bound", "bin": 0},
  {"label": "CAP bound, repressor not bound", "bin": 2}]}
{{< /sortbins >}}

### Describe why regulation of operons is important

{{< multiplechoice
  question="Which of the following is a type of regulation of gene expression unique to eukaryotes?"
  answer="chemical modification of histones"
  hint="Reread the Global Responses, Attenuation and Riboswitches, and DNA-Level Control discussions."
>}}
attenuation
...
{{< textin
  question="The prevention of expression of operons encoding substrate use pathways for substrates other than glucose when glucose is present is called ________."
  answer="catabolite repression"
  hint="Reread the last sentences of the subsection on activation of the lac operon."
>}}

{{< selfcheck question="What are two ways that bacteria can influence the transcription of multiple different operons simultaneously in response to a particular environmental condition?" hint="The answer spans the two subsections of Global Responses of Prokaryotes." >}}
