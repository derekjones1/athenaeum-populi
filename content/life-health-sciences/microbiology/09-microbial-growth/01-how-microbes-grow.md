...
description: >-
  Binary fission and generation time, the four phases of the bacterial growth
  curve, the direct and indirect laboratory methods used to count cells,
  alternative patterns of cell division, and how biofilms form and coordinate
  their activity through quorum sensing — adapted from OpenStax Microbiology,
  Section 9.1.
source_section: "9.1"
weight: 1
---
...
The case continues in [Oxygen Requirements for Microbial Growth](/life-health-sciences/microbiology/09-microbial-growth/02-oxygen-requirements-for-microbial-growth/).
{{< /callout >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_binaryfiss" alt="(a) A false-color scanning electron micrograph of two pink rod-shaped cells joined end to end, appearing as a single dividing bacterium. (b) A four-panel diagram of binary fission: a round cell with one loop of DNA; the cell elongates as its DNA replicates into two loops; a septum forms across the elongated cell's middle; two round cells separate, each with one DNA loop." longdesc="Panel (a) carries a 2 µm scale bar, about as long as one of the two joined cells. Panel (b) shows four stages connected by arrows, left to right. First, a single round cell containing one small loop of DNA. Second, labeled cell elongation and DNA replication, the cell has grown longer and now contains two DNA loops that still overlap at its center, one reaching toward each end. Third, labeled formation of division septum, the elongated cell has pinched inward at its center into a figure-eight shape while still holding the two DNA loops. Fourth, labeled cell separation, two round daughter cells have fully separated, each containing one DNA loop." kind="diagram" eager="true" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_Zring" alt="A three-stage diagram showing the FtsZ ring pinching a dividing cell in two, from an intact ring at the cell's constriction through a septum to full separation." longdesc="Stage 1: a figure-eight-shaped cell with a loop of DNA in each lobe; a ring of green dots circles the pinched middle, labeled cleavage furrow and FtsZ ring. Stage 2: a new wall now runs straight across the cell's middle, and the green dots have lined up in a short single row at its center; two leader lines, both labeled septum, point to this dividing wall, and each lobe still holds a DNA loop. Stage 3: two separate round cells, each containing one DNA loop and a few scattered green dots, with no ring remaining." kind="diagram" >}}
...
{{< textin question="What is the name of the protein that assembles into a Z ring to initiate cytokinesis and cell division?" answer="FtsZ" accept="FtsZ protein" hint="Name the protein that assembles into the Z ring, given a few sentences after binary fission is introduced." >}}
...
The number of cells increases exponentially and can be expressed as 2ⁿ, where *n* is the number of generations. If cells divide every 30 minutes, after 24 hours, 48 divisions would have taken place. If we apply the formula 2ⁿ, where *n* is equal to 48, the single cell would give rise to $2^{48}$ or 281,474,976,710,656 cells at 48 generations (24 hours). When dealing with such huge numbers, it is more practical to use scientific notation. Therefore, we express the number of cells as $2.8 \times 10^{14}$ cells.
...
*N*ₙ is the number of cells at any generation *n*, *N*₀ is the initial number of cells, and *n* is the number of generations.

{{< mediafigure src="microbiology/OSC_Microbio_09_01_generation" alt="A table and branching diagram of exponential growth by binary fission: the table pairs generation numbers 0 through 3 with cell numbers 1, 2, 4, and 8; the diagram shows one cell branching into two, each of those into two more, and each of those into two more." longdesc="The table lists generation 0 with 1 cell, generation 1 with 2 cells, generation 2 with 4 cells, and generation 3 with 8 cells. The tree diagram beside it, under the heading Each division adds two new cells, has each row of cells level with its table row and shows a single cell at the top with two arrows branching down to two cells (generation 1); each of those two cells has two arrows branching down to a total of four cells (generation 2); each of those four cells has two arrows branching down to a total of eight cells (generation 3)." kind="diagram" >}}
...
{{< multiplechoice
  question="With a doubling time of 30 minutes and a starting population size of $1 \times 10^5$ cells, how many cells will be present after 2 hours, assuming no cell death?"
  answer="1,600,000 cells"
  hint="Two hours at a 30-minute doubling time is four generations; apply Nₙ = N₀2ⁿ with $N_0 = 1 \times 10^5$ and n = 4."
>}}
400,000 cells
800,000 cells
1,600,000 cells
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_growthcurv" alt="A line graph of the logarithm of living bacterial cells (y-axis) against time (x-axis). The curve is flat and low at first, then rises steeply, then flattens into a plateau, then declines. Four labeled callouts mark, in order: (1) lag phase — no increase in living cells; (2) log phase — exponential increase; (3) stationary phase — plateau, division and death roughly equal; (4) death or decline phase — exponential decrease." longdesc="Reading left to right along the curve: the line starts low and flat, labeled (1) lag phase: no increase in the number of living bacterial cells. It then rises steeply, labeled (2) log phase: exponential increase in the number of living bacterial cells. It then flattens into a plateau, labeled (3) stationary phase: plateau in the number of living bacterial cells, with the rate of cell division and death roughly equal. It then slopes downward, labeled (4) death or decline phase: exponential decrease in the number of living bacterial cells." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_Jgrowthcur" alt="Two line graphs of log-phase population growth for a culture with a doubling time of 1 hour, both plotting the same three points — (1,2), (5,32), and (10,1024) — for time in hours. (a) On an arithmetic scale for number of cells, the line stays low and nearly flat before curving sharply upward near the end. (b) On a semilogarithmic scale for log₁₀ of number of cells, the same three points fall on a straight diagonal line." longdesc="Both x-axes are marked 0 to 11 hours. (a) The arithmetic-scale graph plots number of cells, with the y-axis marked 0, 100, and 1000, against time in hours; the curve passes through (1, 2), (5, 32), and (10, 1,024), staying close to the horizontal axis until about hour 7 and then rising steeply to the last point. (b) The semilogarithmic-scale graph plots log₁₀ of number of cells against time in hours, using a logarithmic y-axis marked 10⁰, 10¹, 10², and 10³ with fine gridlines between; the same three points, (1, 2), (5, 32), and (10, 1,024), fall on a single straight diagonal line from lower left to upper right. Both graphs are labeled with the equation N = 2ⁿ." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_chemostat" alt="A diagram of a chemostat: a covered vat of fluid with a central vertical shaft ending in two two-bladed (bow-tie shaped) impellers stacked one above the other near the bottom, a curved arrow near the top of the shaft showing it rotating, small bubbles near the impellers, a feed line entering the top on one side, and an effluent line leaving the top on the other side." longdesc="An arrow labeled feed enters through the top left of the vat's lid and points down toward the fluid, its arrowhead ending in the air space just above the surface; on the right, a pipe whose open lower end sits at the fluid surface rises out through the lid and turns into an arrow labeled effluent, pointing away. A vertical shaft runs down the center of the vat, with a curved arrow near its top indicating rotation, to two two-bladed (bow-tie shaped) impellers stacked one above the other near the bottom, and small bubbles are drawn in the fluid above the impellers on both sides." kind="diagram" >}}
...
{{< textin question="During which phase does growth occur at the fastest rate?" answer="log phase" accept="logarithmic phase|logarithmic growth phase|log growth phase|exponential growth phase|exponential phase|logarithmic|exponential|log" hint="Reread the phase subsections under The Growth Curve." >}}

{{< selfcheck question="Name two factors that limit microbial growth." hint="The Stationary Phase discussion names the unfavorable conditions that accumulate as a culture's growth rate slows." >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_hemcy" alt="(a) A gloved hand holds a thick, frosted counting slide embossed REICHERT and HEMACYTOMETER, IMPROVED NEUBAUER, 0.1 mm DEEP; an H-shaped channel cut across its center frames two counting platforms, which look reddish brown against the dark background; the grid itself is too fine to see at this scale. (b) A diagram of the grid, with 0.05 mm, 0.25 mm, and 1.00 mm line-spacing brackets: a magnified circle shows a bold-outlined square subdivided by fine lines into a 4×4 array of smaller squares, with red oval cells scattered inside and around it." longdesc="Panel (b)'s grid carries line-spacing brackets at left and right: 0.05 mm between the finest lines, 0.25 mm across a wider span, and 1.00 mm, repeated three times, across the coarse grid's larger squares. A leader from the label 'Count cells in this square' ends at a small red circle on one square in the grid's densely ruled center, and a gray magnifying wedge runs from that circle to an enlarged circle at right, which shows the bold-outlined square used for counting, subdivided by fine gridlines into a 4×4 array of smaller squares. Ten red oval cells lie inside the bold square's border; four more cells are visible just outside it, in the neighboring squares shown for context, and are not part of the count." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_livedead" alt="A fluorescence micrograph on a black background showing many glowing green rod- and coccus-shaped cells and fewer glowing red or orange cells of similar shapes, with a 100 µm scale bar and the caption text Serratia, Corynebacterium, Staphylococcus printed in the corner." kind="photo" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_coulter" alt="(a) A diagram of a Coulter counter: a beaker of fluid with green dots for cells holds an inner tube with a closed, rounded bottom; a gap in the tube's wall, marked by a purple circle, is the aperture. A lead marked + hangs in the beaker outside the tube and a lead marked − inside it, and curved arrows run from + through the aperture to −. One wire runs from an electronic counter box over the beaker's rim and down inside the tube, ending at the aperture. (b) A photograph of a small glass Coulter counter aperture tube standing in a beaker." longdesc="In panel (a), the outer beaker holds the cell suspension, shown as scattered green dots. Inside it stands a tube, open at the top with a closed, rounded bottom, that also holds a few green dots. The tube's right wall has a small gap, the aperture, marked by a purple circle, with one green dot sitting in it. A lead marked + hangs in the beaker fluid to the right, outside the tube; a lead marked − hangs inside the tube, just above the aperture. Curved arrows trace a path from the + lead down to the aperture, through it, and up to the − lead. A single black wire runs from an electronic counter box at lower left (a numeric display with red and white buttons), loops over the beaker's left rim, and runs down inside the tube to end at the aperture; no wire is drawn to either lead." kind="diagram" >}}
...
{{< multiplechoice
  question="Why would you count the number of cells in more than one square in the Petroff-Hausser chamber to estimate cell numbers?"
  answer="Cells in several small squares must be counted and the average taken to obtain a reliable measurement."
  hint="Reread the paragraph just after the Petroff-Hausser chamber figure."
>}}
Counting more than one square corrects the sample for any dilution performed before it was loaded onto the chamber.
Counting more than one square distinguishes live cells from dead cells and debris of the same size.
Cells in several small squares must be counted and the average taken to obtain a reliable measurement.
Counting more than one square is required so the reading can be compared against a turbidity calibration curve.
{{< /multiplechoice >}}

{{< selfcheck question="In the viability staining method, why do dead cells appear red?" hint="Recall which stain can cross an intact membrane and which can only enter a damaged one, and what happens to the two stains together on a dead cell's nucleic acids." >}}
...
===CHECKS===
dead cells appear red because the red stain displaces the green stain on their nucleic acids
the primary (green) stain can penetrate intact cytoplasmic membranes and stains both live and dead cells
the secondary (red) stain can enter a cell only if its cytoplasmic membrane is considerably damaged
{{< /selfcheck >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_serialtub" alt="A diagram of five serial tenfold dilutions of a bacterial culture, each plated at 0.1 mL, with the resulting colony counts: too numerous to count, too numerous to count, 389 colonies, 50 colonies, and 2 colonies." longdesc="A bottle of dark culture is diluted 1.0 mL into 9.0 mL of broth across five successive tubes, each transfer taking 1.0 mL from the previous tube, producing dilutions of 1:10, 1:100, 1:1,000, 1:10,000, and 1:100,000; each tube is visibly lighter in color than the one before it. A 0.1 mL sample from each tube is spread on its own plate: the 1:10 and 1:100 plates are too numerous to count, the 1:1,000 plate shows 389 colonies, the 1:10,000 plate shows 50 colonies, and the 1:100,000 plate shows 2 colonies." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_pourplate" alt="A four-step diagram of the pour plate method: mixing a bacterial sample with warm agar; pouring the sample onto a sterile plate; swirling the plate and letting it solidify; incubating the plate until colonies appear." longdesc="Step 1: a drop of bacterial sample falls from a pipette above into a test tube already partly filled with warm agar (45–50 °C). Step 2: the tube's mixed contents are poured into an empty sterile Petri dish. Step 3: the dish is swirled to mix, shown by a spiral arrow over the spread agar, and left to solidify. Step 4: the solidified plate is incubated, shown with about twenty scattered round colonies of varying sizes growing across its surface." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_spreadplate" alt="A three-step diagram of the spread plate method: pouring a 0.1 mL bacterial dilution onto solid medium; spreading the sample evenly with a spreader; incubating the plate until colonies grow on the surface." longdesc="Step 1: 0.1 mL of a bacterial dilution, labeled bacterial dilution, is poured from a test tube onto a plate of solid medium. Step 2: a bent glass spreader moves back and forth across the plate's surface to spread the sample evenly, shown by a double-headed arrow. Step 3: the incubated plate shows round colonies scattered across the surface of the medium." kind="diagram" >}}
...
Watch [this video](https://openstax.org/l/22serdilpltcvid) for demonstrations of serial dilutions and spread plate techniques.
{{< /callout >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_MPNtubes" alt="A diagram of the most-probable-number method: three sets of five lactose broth tubes, inoculated with 10 mL, 1 mL, and 0.1 mL of pond water, incubated, and read for a red-to-yellow color change indicating gas and fermentation, with 5 of 5, 2 of 5, and 0 of 5 tubes positive." longdesc="An Erlenmeyer flask of pond water is used to inoculate three sets of five tubes: the first five tubes each receive 10 mL of sample, the next five each receive 1 mL, and the last five each receive 0.1 mL; all tubes start out the same pink color, each with a small upright inner tube also filled with pink broth. After incubation at 37 °C for 24 hours, all 5 of the 10-mL tubes have turned yellow (5 positive tubes), 2 of the 5 1-mL tubes have turned yellow while 3 remain pink (2 positive tubes), and all 5 of the 0.1-mL tubes remain pink (0 positive tubes). In every yellow tube the small inner tube is drawn white and empty, showing trapped gas; in every pink tube it stays filled with pink broth." kind="diagram" >}}
...
{{< multiplechoice
  question="What is a colony-forming unit?"
  answer="A count expressed per milliliter that represents a colony, which may have arisen from more than one cell landing on the same spot rather than always a single cell."
  hint="Reread the opening paragraph of Plate Count."
>}}
An individual bacterial cell counted directly under a microscope on a calibrated slide.
A statistical estimate of the number of live cells obtained from a pattern of positive and negative tubes.
A unit of turbidity used to estimate cell density with a spectrophotometer.
A count expressed per milliliter that represents a colony, which may have arisen from more than one cell landing on the same spot rather than always a single cell.
{{< /multiplechoice >}}

{{< selfcheck question="What two methods are frequently used to estimate bacterial numbers in water samples?" hint="One method is named a paragraph later as often used for water and food samples; the other is named a section earlier for concentrating very dilute samples such as drinking water before plating." >}}
...
===CHECKS===
the original sample is instead concentrated using the membrane filtration technique, in which known volumes are vacuum-filtered aseptically through a membrane that traps microorganisms
microbiologists also routinely use the most probable number (MPN) method, a statistical procedure often used for water and food samples
{{< /selfcheck >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_spectroph" alt="(a) A photograph of a benchtop spectrophotometer with a keypad, screen, and sample chamber. (b) A diagram of light passing from a source through a prism into a wavelength selector, then through a control tube of clear broth to a detector at full strength, and through a turbid sample tube to a detector at reduced strength, with the beam scattering inside the turbid tube." longdesc="In panel (b), a light bulb shines into a triangular prism, which splits the light into a rainbow-colored fan; a wavelength-selection slit picks out one color. In the top row, this single-wavelength beam passes through a tube of clear yellow broth largely undiminished and reaches a detector box at full width. In the bottom row, the same beam passes through a tube of more turbid yellow broth, where arrows show the beam scattering inside the tube, and only a narrower beam continues on to the detector." kind="diagram" >}}
...
{{< multiplechoice
  question="What is the purpose of a calibration curve when estimating cell count from turbidity measurements?"
  answer="It lets turbidity readings be converted into cell-count estimates, for samples obtained or cultured under similar conditions and within the density range used to build the curve."
  hint="Reread the turbidity paragraph under Indirect Cell Counts."
>}}
It corrects a diluted sample's cell count back to the concentration of the original undiluted culture.
It selects which wavelength of light the spectrophotometer should use for the measurement.
It distinguishes live cells from dead cells within the sample being measured.
It lets turbidity readings be converted into cell-count estimates, for samples obtained or cultured under similar conditions and within the density range used to build the curve.
{{< /multiplechoice >}}

{{< selfcheck question="What are the newer indirect methods of counting live cells?" hint="The Indirect Cell Counts discussion names three kinds of metabolic products or reactants that can be monitored to estimate cell number." >}}
...
===CHECKS===
they measure cell activity by following the production of metabolic products or the disappearance of reactants
adenosine triphosphate (ATP) formation, biosynthesis of proteins and nucleic acids, and consumption of oxygen can all be monitored to estimate the number of cells
{{< /selfcheck >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_01_filam" alt="(a) A light micrograph of many long, thin, beaded chains of blue-green cells crossing over one another. (b) An electron micrograph of a larger round cell attached to a smaller round cell; the larger cell's darker interior region is labeled N and the smaller cell, at lower left, has its own darker region labeled N, and an arrow labeled NE points to the left edge of the smaller cell." longdesc="Panel (b) shows a large mother cell and a smaller budding cell attached to it, plus additional partial cells at the image edges. Inside the mother cell, a dark, irregular internal region is labeled N (nucleoid). The smaller attached cell, at lower left, also has a dark internal region labeled N, and an arrow labeled NE, coming in from the left, points to the outer boundary of that smaller cell, identifying the still-forming nuclear envelope." kind="photo" >}}
...
{{< selfcheck question="Identify at least one difference between fragmentation and budding." hint="Compare how many new cells each process produces at once and where on the parent cell each process occurs." >}}
...
===CHECKS===
many nucleoids accumulate in an enlarged round cell or along a filament
the resulting new cells split away from the parent filament all at once
a single long narrow extension forms at one pole of the cell
the tip of that extension swells into one smaller bud that eventually detaches from the parent cell
{{< /selfcheck >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_05_filmdiag" alt="A five-stage diagram of biofilm formation on a surface, from a few scattered flagellated cells through small clusters, larger clusters, very large clusters with holes, to a large mature colony releasing free-swimming cells." longdesc="Five numbered boxes above a gray surface. Stage 1, reversible attachment of planktonic cells (seconds): a few flagellated cells sit loosely on the surface with no matrix around them. Stage 2, first colonizers become irreversibly attached (second, minutes): small clusters of cells lie in thin golden-yellow patches of matrix on the surface. Stage 3, growth and cell division (hours, days): larger, flat clusters in golden matrix, with small white openings on top, cover more of the surface. Stage 4, production of EPS and formation of water channels (hours, days): very large mushroom-shaped towers of cells held in golden matrix, with white openings (channels) through them. Stage 5, attachment of secondary colonizers and dispersion of microbes to new sites (days, months): from the large mature cluster, small flagellated cells are shown detaching and swimming away." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_09_05_autoinduc" alt="Two chemical structures side by side. Left, a short peptide labeled short peptide: a chain of amino acids (Tyr-Ser-Thr-Cys) with Cys also linked through a sulfur atom into a ring formed with Asp-Phe-Ile-Met and a carbon bearing a double-bonded oxygen. Right, a structure labeled N-acetylated homoserine lactone: a five-membered ring containing one oxygen, with a double-bonded oxygen on the ring carbon beside it, and a nitrogen atom outside the ring linked to an R group and to a carbon bearing both a double-bonded oxygen and a hydrogen." longdesc="The left structure, labeled short peptide, is a chain of amino acids reading Tyr-Ser-Thr-Cys, with the Cys side chain's sulfur bonded into a ring that continues Cys-Asp-Phe-Ile-Met, closing back to a carbonyl carbon (a carbon with a double-bonded oxygen) bonded to the sulfur. The right structure, labeled N-acetylated homoserine lactone, is a five-membered ring holding one ring oxygen; the ring carbon next to that oxygen carries a double-bonded oxygen outside the ring (the lactone's C=O), and a nitrogen outside the ring is attached to the next ring carbon and carries an R group on one side and, on the other, a carbon bonded to a double-bonded oxygen and a hydrogen." kind="diagram" >}}
...
Previous approaches to prevent biofilm build-up included treating surfaces with anti-bacterial substances. But researchers Pamela Flores at University of Colorado and Samantha McBride of MIT described in a 2023 paper, these often failed when a layer of dead biofilm cells forms and becomes an anchor point for subsequent biofilms to grow. Furthermore, rapid microbial adaptation leads to resistant strains, which may lead to even greater risks for crew and equipment. So, instead of killing microorganisms, McBride and Flores—working with Kripa Varanasi (MIT), Luis Zea (U. of Colorado), and Jonathan Galazka (NASA)—treated surfaces with a layer of lubricant, which was infused into the tiny crevices of the surface and held there by capillary action. Experiments on Earth and on the ISS showed that the lubrication was effective at preventing biofilm growth because it eliminated microorganism adhesion. Beyond space exploration applications, the approach may be used in medical and other devices. (Flores, P., McBride, S.A., Galazka, J.M. et al. (2023). Biofilm formation of *Pseudomonas aeruginosa* in spaceflight is minimized on lubricant impregnated surfaces. *Nature Microgravity* 9, 66.)

**Check Your Understanding**

{{< textin question="What is the matrix of a biofilm composed of?" answer="extracellular polymeric substances" accept="EPS" hint="Name the hydrated-gel substance secreted by the biofilm's organisms whose channels keep the biofilm hydrated and that shelters its cells from predation." >}}

{{< multiplechoice
  question="What is the role of quorum sensing in a biofilm?"
  answer="It lets cells in the biofilm coordinate their activities in response to environmental stimuli."
  hint="Reread the quorum-sensing paragraph under Biofilm Formation."
>}}
It triggers the initial attachment of planktonic cells to a conditioning film on the substrate.
It lets cells in the biofilm coordinate their activities in response to environmental stimuli.
It shelters biofilm organisms from predation by protozoans and white blood cells.
It allows biofilm cells to exchange extrachromosomal DNA carrying antibiotic-resistance genes.
{{< /multiplechoice >}}
...
{{< multiplechoice
  question="Which of the following is the best definition of generation time in a bacterium?"
  answer="the length of time it takes for a population of cells to double"
  hint="Reread the opening paragraph of Generation Time."
>}}
...
{{< multiplechoice
  question="What is the function of the Z ring in binary fission?"
  answer="It forms a contractile ring at the septum."
  hint="Reread the FtsZ sentences in Binary Fission and the Z-ring figure caption."
>}}
...
{{< multiplechoice
  question="If a culture starts with 50 cells, how many cells will be present after five generations with no cell death?"
  answer="1600"
  hint="Apply Nₙ = N₀2ⁿ with N₀ = 50 and n = 5."
>}}
200
400
1600
...
{{< multiplechoice
  ...
  answer="death phase"
  hint="Compare what the last two phase subsections of The Growth Curve say about endospores."
>}}
death phase
lag phase
log phase
...
{{< multiplechoice
  question="During which phase would penicillin, an antibiotic that inhibits cell-wall synthesis, be most effective?"
  answer="log phase"
  hint="Recall which phase the module names as the stage where bacteria are most susceptible to antibiotics that affect cell-wall synthesis."
>}}
death phase
lag phase
log phase
...
{{< multiplechoice
  question="Which growth phase matches this description: the number of dying cells is higher than the number of cells dividing?"
  answer="Death phase"
  hint="Recall which phase is defined by dying cells outnumbering dividing cells."
>}}
Lag phase
Log phase
Stationary phase
...
{{< multiplechoice
  question="Which growth phase matches this description: the number of new cells is equal to the number of dying cells?"
  answer="Stationary phase"
  hint="Recall which phase is defined by cell division and cell death being roughly balanced."
>}}
Lag phase
Log phase
Stationary phase
...
{{< multiplechoice
  question="Which growth phase matches this description: new enzymes to use available nutrients are induced?"
  answer="Lag phase"
  hint="Reread the four phase subsections under The Growth Curve."
>}}
Lag phase
Log phase
Stationary phase
...
{{< multiplechoice
  question="Which growth phase matches this description: binary fission is occurring at maximum rate?"
  answer="Log phase"
  hint="Reread the four phase subsections under The Growth Curve."
>}}
Lag phase
...
{{< textin question="The ________ method allows direct count of total cells growing on solid medium." answer="plate count" accept="viable plate count|total plate count" hint="Name the method that counts colonies formed on solid medium, distinct from the calibrated-slide or electronic counting methods." >}}

{{< multiplechoice
  question="Which of the following methods would be used to measure the concentration of bacterial contamination in processed peanut butter?"
  answer="total plate count"
  hint="Reread Measurement of Bacterial Growth."
>}}
...
{{< multiplechoice
  question="Which pair of tools is used to perform a direct count of total cells?"
  answer="hemocytometer, Petroff-Hausser counting chamber"
  hint="Reread Direct Cell Count."
>}}
hemocytometer, Petroff-Hausser counting chamber
spectrophotometer, Coulter counter
pour plate, spread plate
serial dilution tube, membrane filter
{{< /multiplechoice >}}

{{< textin question="A statistical estimate of the number of live cells in a liquid is usually done by ________." answer="most probable number" accept="MPN|most probable number method|MPN method|most probable number (MPN)" hint="Reread the last subsection of Plate Count." >}}

{{< textin question="For this indirect method of estimating the growth of a culture, you measure ________ using a spectrophotometer." answer="turbidity" hint="Name the cloudiness measurement the spectrophotometer detects as an indirect indicator of cell density." >}}

{{< multiplechoice
  question="Active growth of a culture may be estimated indirectly by measuring which of the following products of cell metabolism?"
  answer="ATP, acid from fermentation"
  hint="Reread the last paragraph of Indirect Cell Counts."
>}}
turbidity, dry weight
ATP, acid from fermentation
colony-forming units, most probable number
oxygen consumption, nucleic acid degradation
{{< /multiplechoice >}}

{{< selfcheck question="Why is it important to measure the transmission of light through a control tube with only broth in it when making turbidity measures of bacterial cultures?" hint="Combine what the spectrophotometer measures the sample against with the caption's note about how much the medium alone scatters light." >}}
...
===CHECKS===
measures the filtered light that passes through the sample against a control with only medium
the medium and vessel alone still scatter some light
the additional decrease in transmission (and increase in absorbance) seen with the bacterial sample can be attributed to the turbidity the cells themselves cause
{{< /selfcheck >}}

{{< selfcheck question="In terms of counting cells, what does a plating method accomplish that an electronic cell counting method does not?" hint="Contrast what the viable plate count is a count of with the one limitation named for the Coulter counter." >}}
A plating method, such as the viable plate count, counts only viable or live cells, because it depends on a cell actually replicating and giving rise to a visible colony under suitable incubation conditions. An electronic cell counting method such as the Coulter counter, by contrast, counts every particle that passes through its aperture and changes the resistance reading — it does not differentiate between live and dead cells.
===CHECKS===
counts only viable or live cells
giving rise to a visible colony under suitable incubation conditions
counts every particle that passes through its aperture and changes the resistance reading
it does not differentiate between live and dead cells
{{< /selfcheck >}}

{{< selfcheck question="Refer to Appendix B. If the results from a pond water sample were recorded as 3, 2, 1, what would be the MPN of bacteria in 100 mL of pond water?" hint="The module explains the method (comparing the pattern of positive tubes to a probability table in Appendix B) but Appendix B's table itself is not reproduced in this section." >}}
...
===CHECKS===
comparing the pattern of positive tubes across the three dilutions to Figure B1 in Appendix B, a table constructed using a probability model of the sampling procedure
this section's own worked example compares the pattern 5, 2, 0 to that table and reads off 49 as the most probable number of bacteria per 100 mL
{{< /selfcheck >}}

{{< selfcheck question="Refer to the spectrophotometer figure above. Why does turbidity lose reliability at high cell concentrations when the culture reaches the stationary phase?" hint="A calibration curve is only trustworthy for samples with densities inside the range used to construct it — recall what that implies about samples outside that range." >}}
...
===CHECKS===
only reliable for estimating cell counts within the range of densities used to construct the curve in the first place
a turbidity reading taken at that concentration is no longer a trustworthy estimate of cell number
{{< /selfcheck >}}

### Describe examples of cell division not involving binary fission, such as budding or fragmentation

{{< multiplechoice
  question="Filamentous cyanobacteria often divide by which of the following?"
  answer="fragmentation"
  hint="Reread Alternative Patterns of Cell Division."
>}}
budding
mitosis
fragmentation
...
{{< multiplechoice
  question="Which pattern of cell division involves a long narrow extension forming at one pole of the cell, with the tip swelling into a smaller cell that eventually detaches from the parent?"
  answer="budding"
  hint="Reread Alternative Patterns of Cell Division."
>}}
fragmentation
binary fission
multiple nucleoid formation
budding
{{< /multiplechoice >}}

{{< textin question="In *Actinomyces*, elongated cells within ________ divide simultaneously to form spores that give rise to new colonies." answer="aerial filaments" hint="Name the structures Actinomyces forms in response to environmental signals, probably related to low nutrient availability." >}}

### Describe the formation and characteristics of biofilms

{{< selfcheck question="Order the following stages of the development of a biofilm from the earliest to the last step: (A) secretion of EPS, (B) reversible attachment, (C) dispersal, (D) formation of water channels, (E) irreversible attachment." hint="Match each lettered stage to the numbered stages shown in the biofilm-formation diagram above." >}}
...
===CHECKS===
reversible attachment (B) is the first stage, in which planktonic cells attach to a conditioning film on the substrate
Irreversible attachment (E) follows, as cells express new phenotypes that let them transition to a sessile lifestyle
Secretion of EPS (A) and formation of water channels (D) occur together as the biofilm develops its characteristic matrix and channel structures
Dispersal (C) is the last stage, when cells on the periphery revert to a planktonic lifestyle and leave to colonize new sites
{{< /selfcheck >}}

{{< textin question="EPS is a hydrated gel composed primarily of ________." answer="polysaccharides" hint="Name the primary macromolecule class of the extracellular matrix, alongside the proteins, nucleic acids, and lipids it also contains." >}}

{{< textin question="Free-floating microbial cells that live in an aquatic environment are called ________ cells." answer="planktonic" hint="Name the term contrasted with sessile once cells attach to a substrate and begin forming a biofilm." >}}

### Identify health risks associated with biofilms and how they are addressed

{{< multiplechoice
  question="Which is a reason for antimicrobial resistance being higher in a biofilm than in free-floating bacterial cells?"
  answer="Cells are metabolically inactive at the base of a biofilm."
  hint="Reread the antibiotic-resistance paragraph in Biofilms and Human Health."
>}}
...
{{< selfcheck question="Infections among hospitalized patients are often related to the presence of a medical device in the patient. Which conditions favor the formation of biofilms on in-dwelling catheters and prostheses?" hint="Combine the general condition the module says will eventually grow a biofilm with the specific in-dwelling devices the module names as biofilm sites." >}}
...
===CHECKS===
pathogens embedded within a biofilm on the catheter, such as *S. epidermidis*, exhibit a higher resistance to antibiotics than their free-floating counterparts
Cells in the deep layers of a biofilm are metabolically inactive and may be less susceptible to the action of antibiotics that disrupt metabolic activities
...
biofilms provide an environment for the exchange of extrachromosomal DNA, which often includes genes that confer antibiotic resistance
removing the intravenous catheter also removed the surface on which the biofilm had formed
{{< /selfcheck >}}

### Describe quorum sensing and its role in cell-to-cell communication and coordination of cellular activities

{{< multiplechoice
  question="Quorum sensing is used by bacterial cells to determine which of the following?"
  answer="the density of the population"
  hint="Recall what autoinducers let cells detect about their own population, once a critical threshold is reached."
>}}
...
{{< multiplechoice
  question="Which of the following statements about autoinducers is incorrect?"
  answer="They bind directly to DNA to activate transcription."
  hint="Reread the paragraph on the signaling molecules of quorum sensing under Biofilm Formation."
>}}
