...
description: >-
  Nucleic acid probes, agarose gel electrophoresis, RFLP analysis, Southern
  and northern blots, microarray analysis, PAGE for proteins, and PCR and DNA
  sequencing — adapted from OpenStax Microbiology, Section 12.2.
source_section: "12.2"
weight: 2
---
...
Part 3 of this case continues later on this page. The case began in [Microbes and the Tools of Genetic Engineering](/life-health-sciences/microbiology/12-modern-applications-of-microbial-genetics/01-microbes-and-the-tools-of-genetic-engineering/).
{{< /callout >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_12_02_RFLP" alt="A diagram of RFLP analysis. A normal DNA strand is cut by the restriction enzyme MstI at three sites; a mutation on a second copy of the strand destroys one of those three sites, leaving only two cut sites. After digestion and gel electrophoresis, the normal strand's lane shows a full set of bands, while the mutated strand's lane is missing two of the shorter bands and instead shows one longer band in their place." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_12_02_Southern" alt="A three-step diagram of a Southern blot. Step 1: electrophoresis separates DNA fragments by size in an agarose gel, with so many fragments that they form a smear. Step 2: the gel sits on filter paper over a hybridization-buffer reservoir, with a nylon membrane, paper towels, and a weight stacked above it; buffer wicks upward through the gel and membrane into the towels, carrying the DNA fragments into the membrane. Step 3: the membrane, now shown alone, is bathed in a solution containing a fluorescently labeled DNA probe, which hybridizes to a few specific locations, shown as dark bands." kind="diagram" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_12_02_Microarray" alt="(a) A flow diagram: cancerous and healthy cells, grown separately, each yield mRNA that reverse transcriptase converts to cDNA labeled red (cancerous) or green (healthy); the two labeled targets are combined and hybridized onto a microarray chip. (b) A heat-map grid of colored cells, with a key: green, only expressed in healthy cells; red, only expressed in cancerous cells; yellow, expressed in both." longdesc="Panel (a), top to bottom: a dish of cancerous cells and a dish of healthy cells are cultured side by side; an arrow labeled 'RNA isolation' leads from each dish to a box labeled mRNA; an arrow labeled 'reverse transcriptase labeling' leads from the cancerous-cell mRNA to a box labeled cDNA under a 'red fluorescent probes' caption, and from the healthy-cell mRNA to a box labeled cDNA under a 'green fluorescent probes' caption; both cDNA boxes feed into a 'combine targets' step and then a 'hybridize to microarray' step, ending at a drawing of a gray microarray chip divided into a grid of squares. Panel (b): a heat-map grid with sample identifiers running down the left side and probe identifiers running along the bottom; most cells are colored green, red, black, or yellow, with a small legend below reading green for genes expressed only in healthy cells, red for genes expressed only in cancerous cells, and yellow for genes expressed in both." kind="diagram" >}}
...
Explore [microchip technology](https://openstax.org/l/22intwebmictec) at this interactive website.
{{< /callout >}}
...
A single-stranded DNA fragment that is complementary to part of the gene of interest
...
transferring the separated DNA fragments out of the gel onto a thin, positively charged membrane
exposed to a labeled DNA probe for detection
{{< /selfcheck >}}
...
Charge only
Size only
Isoelectric point only
Both charge and size together
{{< /multiplechoice >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_12_02_LymeRash" alt="A close-up photo of a forearm with a bull's-eye rash: a solid red spot at the center surrounded by a paler ring of skin and then a fainter red ring." kind="photo" >}}
...
The case continues in [Whole Genome Methods and Pharmaceutical Applications of Genetic Engineering](/life-health-sciences/microbiology/12-modern-applications-of-microbial-genetics/03-whole-genome-methods-and-pharmaceutical-applications-of-genetic-engineering/). Part 2 of this case appears earlier on this page. The case began in [Microbes and the Tools of Genetic Engineering](/life-health-sciences/microbiology/12-modern-applications-of-microbial-genetics/01-microbes-and-the-tools-of-genetic-engineering/).
{{< /callout >}}
...
Deepen your understanding of the polymerase chain reaction by viewing this [animation](https://openstax.org/l/22polychareami) and working through an [interactive](https://openstax.org/l/22intexerpolchr) exercise.
{{< /callout >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_12_02_Sequencer" alt="A four-step diagram summarizing automated Sanger sequencing. Step 1: a PCR tube receives DNA template, primers, DNA polymerase, dNTPs, and fluorescently labeled ddNTPs, shown as ten primed templates, four already extended by a short fragment capped with a colored star. Step 2: elongation either continues (dNTP added) or stops (ddNTP added), producing ten fragments of different lengths, each ending in one of four star colors. Step 3: the fragments run through a color-banded vertical capillary gel past a laser and a detector. Step 4: a computer screen shows the multicolored peak trace." kind="diagram" >}}
...
View an [animation](https://openstax.org/l/22454seqanim) about 454 sequencing to deepen your understanding of this method.
{{< /callout >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_12_02_Ribotyping" alt="A gel with eight lanes of PCR-ribotype reference strains, labeled top to bottom 078, 126, 012, 018, 017, 027, 001, and 020, each showing a distinct pattern of vertical bands, plus a ninth lane at the bottom labeled Javier's whose band pattern matches the 027 reference lane above it." kind="photo" >}}
...
{{< mediafigure src="microbiology/OSC_Microbio_12_02_Ribo_Rep" alt="Left: a diagram of a circular bacterial genome with short primers bound at several repetitive sequences around its circumference; three sets of concentric curved lines in blue, green, and purple radiate outward, representing PCR-amplified fragments of three sizes, and an arrow leads to a schematic gel lane with three bands at those sizes. Right: a photograph of a gel with DNA ladder lanes at both edges (lanes 1 and 11), sizes marked from 506 to 5090 base pairs, and nine lanes of C. difficile strains (2 to 10) between them, each with a distinct multi-band pattern." kind="diagram" >}}
...
natural replication copies an entire genome and initiates at one or more origin sites using primers built during replication
PCR targets a specific, chosen region of a DNA sample using sequence-specific primers
...
whenever a ddNTP is ... incorporated ... it terminates replication of that strand at that point
the fluorescence color of each band ... produces the nucleotide sequence of the template strand
{{< /selfcheck >}}
...
So that the probe becomes single-stranded before hybridizing
...
microarray analysis ... allows for the examination of many thousands of genes at once
each slide can hold more than 30,000 different DNA fragment types
{{< /selfcheck >}}
...
SDS attaches a fluorescent molecular beacon to each protein for visualization
...
Because it must survive the cold temperatures used to store DNA samples
...
at too high a temperature, the primers would not anneal properly to the template strands
