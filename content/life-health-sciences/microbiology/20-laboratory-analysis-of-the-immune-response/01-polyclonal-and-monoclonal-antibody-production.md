...
description: >-
  How antisera and monoclonal antibodies are produced and used for research
  and diagnosis — antibody specificity and cross-reactivity, polyclonal
  antiserum production and its false-positive and false-negative
  limitations, hybridoma-based monoclonal antibody production, and
  humanized antibodies and plantibodies — adapted from OpenStax
  Microbiology, Section 20.1.
source_section: "20.1"
weight: 1
---
...
The case continues in [EIAs and ELISAs](/life-health-sciences/microbiology/20-laboratory-analysis-of-the-immune-response/04-eias-and-elisas/).
{{< /callout >}}
...
this happens because some antigens are chemically similar enough
...
{{< selfcheck question="What is a false positive and what are some reasons that false positives occur?" hint="The definition is one sentence; the cause is named two paragraphs later." >}}
...
===CHECKS===
a false-positive result confirms the presence of an antigen that is not actually present
false positives often occur due to cross-reactivity
cross-reactivity happens when epitopes from a different pathogen resemble those of the pathogen being tested for
{{< /selfcheck >}}

{{< selfcheck question="What is a false negative and what are some reasons that false negatives occur?" hint="The definition is one sentence; the reasons come from a separate paragraph on false negatives, later in this section." >}}
...
===CHECKS===
a false-negative result occurs when the test fails to detect an antibody that is actually present
a false negative can occur when the immune system has not yet produced detectable levels of antibody
for some diseases it may take several weeks to cross the detection threshold
in immunocompromised patients the immune system may not produce a detectable level of antibodies
{{< /selfcheck >}}
...
the variable regions of the mouse genes are ligated to human constant regions
the chimeric gene is transferred into a host cell
...
polyclonal antibodies, by contrast, recognize multiple epitopes on an antigen
monoclonal antibodies recognize a single epitope on an antigen
{{< /selfcheck >}}

{{< callout type="info" >}}
**Micro Connection.** *Using Monoclonal Antibodies to Combat Ebola*
