---
title: Prealgebra
description: The foundations of arithmetic and the first ideas of algebra — the starting rung of the mathematics ladder.
prerequisites: []
reviewed_by: []
license: CC BY-NC-SA 4.0
source: OpenStax Prealgebra 2e
linkTitle: "Overview"
cascade:
  type: docs
---

This is the starting rung of the mathematics ladder: the foundations of
arithmetic and the first ideas of algebra. It assumes no prior coursework and
builds toward Algebra.

{{< callout type="info" >}}
  **Prerequisites:** none. This is a foundational book — a good place to begin.
{{< /callout >}}

## Chapters

This book is organized into chapters, each broken into short sections you can
work through in order.

- **Whole Numbers** — counting, place value, and the four operations.
- **The Language of Algebra** — variables, expressions, and how we write math.

## About this edition

This book is adapted from **OpenStax Prealgebra 2e**, used under the terms of its
**Creative Commons Attribution-NonCommercial-ShareAlike (CC BY-NC-SA 4.0)**
license. Reviewers who vet or adapt this material are credited in the
`reviewed_by` field of each page.
