...
description: >-
  Solving equations using the Division and Multiplication Properties of
  Equality, and solving equations that need to be simplified first —
  adapted from OpenStax Prealgebra 2e, Section 8.2.
source_section: "8.2"
weight: 2
---
...
We introduced the Multiplication and Division Properties of Equality
earlier, modeling how these properties worked using envelopes and counters
and then applying them to solving equations. We restate them again here as
we prepare to use these properties again.
...
{{< callout type="info" >}}
  **Division and Multiplication Properties of Equality.**
  ...
  Multiplication Property of Equality: for all real numbers $a$, $b$, $c$,
  if $a = b$, then $ac = bc$.
{{< /callout >}}

When you divide or multiply both sides of an equation by the same
quantity, you still have equality. Let's review how these properties of
equality can be applied in order to solve equations. Remember, the goal is
to "undo" the operation on the variable. In the example below the variable
is multiplied by $4$, so we will divide both sides by $4$ to "undo" the
multiplication.
...
In that example, to "undo" multiplication, we divided. When the variable
is divided by a number, we "undo" division by multiplying.

**Example.** Solve: $\tfrac{a}{-7} = -42$.

Here $a$ is divided by $-7$. We can multiply both sides by $-7$ to isolate
$a$:

$$-7\left(\tfrac{a}{-7}\right) = -7(-42) \qquad\qquad \tfrac{-7a}{-7} = 294$$
...
Remember $-r$ is equivalent to $-1r$. Rewrite $-r$ as $-1r$, then divide
both sides by $-1$:

$$-1r = 2 \qquad\qquad \tfrac{-1r}{-1} = \tfrac{2}{-1}$$

$$r = -2$$

Check: substitute $r = -2$ into $-r = 2$:

$$-(-2) \stackrel{?}{=} 2 \qquad 2 = 2 \checkmark$$

We saw earlier that there are two other ways to solve $-r = 2$: we could
multiply both sides by $-1$, or we could take the opposite of both sides.
...
**Example.** Solve: $\tfrac{2}{3}x = 18$.

Since the product of a number and its reciprocal is $1$, our strategy will
be to isolate $x$ by multiplying by the reciprocal of $\tfrac{2}{3}$.
Multiply both sides by $\tfrac{3}{2}$, the reciprocal of $\tfrac{2}{3}$:

$$\tfrac{3}{2} \cdot \tfrac{2}{3}x = \tfrac{3}{2} \cdot 18$$

Reciprocals multiply to one, so the left side becomes $1x$:

$$1x = \tfrac{3}{2} \cdot \tfrac{18}{1}$$

Multiply:

$$x = 27$$

Check: substitute $x = 27$ into $\tfrac{2}{3}x = 18$:

$$\tfrac{2}{3} \cdot 27 \stackrel{?}{=} 18 \qquad 18 = 18 \checkmark$$

Notice that we could have divided both sides of the equation
$\tfrac{2}{3}x = 18$ by $\tfrac{2}{3}$ to isolate $x$. While this would
work, multiplying by the reciprocal requires fewer steps.
...
Many equations start out more complicated than the ones we've just solved.
First, we need to simplify both sides of the equation as much as possible.

**Example.** Solve: $8x + 9x - 5x = -3 + 15$.

Start by combining like terms to simplify each side:

$$8x + 9x - 5x = -3 + 15 \quad\longrightarrow\quad 12x = 12$$

Divide both sides by $12$ to isolate $x$:

$$\tfrac{12x}{12} = \tfrac{12}{12} \qquad\qquad x = 1$$
...
**Example.** Solve: $11 - 20 = 17y - 8y - 6y$.

Simplify each side by combining like terms:

$$11 - 20 = 17y - 8y - 6y \quad\longrightarrow\quad -9 = 3y$$
...
solution with the variable on the left side.
...
$$\tfrac{-3n}{-3} = \tfrac{21}{-3} \qquad\qquad n = -7$$
