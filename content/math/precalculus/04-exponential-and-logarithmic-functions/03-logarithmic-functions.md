...
Currently, we use $\log(x)$, $\text{lg}(x)$ as the common logarithm, $\text{lb}(x)$ as the binary logarithm, and $\ln(x)$ as the natural logarithm. Writing $\text{lg}(x)$ without specifying a base is now considered bad form, despite being frequently found in older materials.
